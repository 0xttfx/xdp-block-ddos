...
if [ ! -f xdp_ddos_blocker.o ]; then
    echo "Erro: Arquivo 'xdp_ddos_blocker.o' não encontrado. Compile o programa primeiro."
    exit 1
fi
echo "Arquivo objeto encontrado."

# Define o caminho de fixação (pin) do programa eBPF
PIN_PATH="/sys/fs/bpf/xdp_ddos_blocker"

echo "=== Teste: Carregando o programa XDP com bpftool ==="
# Carrega o programa XDP e o fixa (pin) no sistema de arquivos BPF
sudo bpftool prog load xdp_ddos_blocker.o $PIN_PATH type xdp
...
if sudo bpftool prog show | grep -q xdp_ddos_blocker; then
